    ...
    if (width > zoom_limit || height > zoom_limit || zoom_factor < 0) {
        if (zoom_factor < 0.99f) {
            ...
            if (!newPos.Equals(target)) {
                // Note that we call SetPosition directly on the control. 3dgs recalculates the field of view after setting camera
                // position (which this code is relying on), but HPS will stomp on the fov and replace it with the old value from
                // the kit if we set the whole kit.
